(() => {
  ...
  const serviceLinks = {
    '/services/end-of-lease-cleaning/': [['Regular house cleaning','/services/regular-house-cleaning/'], ['Deep cleaning','/services/deep-cleaning/'], ['Move-in cleaning','/services/move-in-cleaning/'], ['Request a quote','/contact/']],
    '/services/regular-house-cleaning/': [['Weekly cleaning','/services/weekly-cleaning/'], ['Fortnightly cleaning','/services/fortnightly-cleaning/'], ['Deep cleaning','/services/deep-cleaning/'], ['Request a quote','/contact/']],
    '/services/deep-cleaning/': [['Regular house cleaning','/services/regular-house-cleaning/'], ['End-of-lease cleaning','/services/end-of-lease-cleaning/'], ['Once-off cleaning','/services/once-off-cleaning/'], ['Request a quote','/contact/']],
  ...
  const cleanText = () => {
    const replacements = {
      'Preview Optimized SEO Landing': 'View service details',
      'Special Estimate Quote': 'Quote on request',
      'Trust Is Our Core Core Currency': 'Trust is our core currency'
    };
  ...
  const addRelatedLinks = () => {
    ...
    const items = serviceLinks[path];
    if (!items || document.querySelector('[data-seo-related-links]')) return;
    const section = document.createElement('section');
    section.dataset.seoRelatedLinks = 'true';
    section.className = 'seo-related-links';
    section.innerHTML = `<h2>Related cleaning services</h2><p>${items.map(([label, href]) => `<a href="${href}">${label}</a>`).join(' · ')}</p>`;
