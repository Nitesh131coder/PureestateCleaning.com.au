(() => {
  ...
  const swap = () => {
    document.querySelectorAll('button').forEach(button => {
      ...
      const a = document.createElement('a'); a.href = href; a.className = button.className; a.innerHTML = button.innerHTML; a.dataset.urlified = 'true'; button.replaceWith(a);
    });
    const logo = document.querySelector('header div.cursor-pointer.group');
    if (logo && !logo.dataset.urlified) { const a = document.createElement('a'); a.href='/'; a.className=logo.className; a.innerHTML=logo.innerHTML; a.dataset.urlified='true'; logo.replaceWith(a); }
